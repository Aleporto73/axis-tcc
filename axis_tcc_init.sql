-- =====================================================
-- AXIS TCC - DATABASE INITIALIZATION
-- Version: v1.3-FINAL
-- =====================================================
...
CREATE INDEX idx_exposure_items_hierarchy ON exposure_items(hierarchy_id, position);
...
-- ⚠️ REGRA: NUNCA UPDATE, SEMPRE INSERT
CREATE INDEX idx_clinical_states_patient ON clinical_states(patient_id, created_at DESC);
...
CREATE INDEX idx_sessions_tenant ON sessions(tenant_id, scheduled_at DESC);
...
CREATE INDEX idx_tasks_patient ON tasks(patient_id, assigned_at DESC);
CREATE INDEX idx_tasks_status ON tasks(status);
CREATE INDEX idx_tasks_hierarchy ON tasks(hierarchy_id);
...
-- ⚠️ REGRA: imutável após reviewed=true
...
CREATE INDEX idx_suggestions_patient ON suggestions(patient_id, created_at DESC);
...
-- ⚠️ REGRA: APPEND-ONLY, NUNCA UPDATE/DELETE
CREATE INDEX idx_suggestion_decisions_patient ON suggestion_decisions(patient_id, created_at DESC);
...
CREATE INDEX idx_assist_suggestions_status ON assist_suggestions(status);

-- =====================
-- ASSIST AUDIT LOG
-- =====================
CREATE TABLE assist_audit_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES tenants(id),
...
-- ⚠️ REGRA: APPEND-ONLY
CREATE INDEX idx_assist_audit_log_suggestion ON assist_audit_log(assist_suggestion_id);
...
-- VERIFICATION
-- =====================
SELECT 'AXIS TCC Database initialized successfully!' AS status;

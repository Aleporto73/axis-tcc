-- Migration 015: Seed Biblioteca de Protocolos ABA
-- IDs das ebp_practices (integer, nomes em português no banco)
--   7  = Ensino por tentativas discretas (DTT)
--   14 = Intervenção naturalista (NET)
--   2  = Comunicação alternativa e aumentativa (PECS)
--   22 = Narrativas sociais (Social Stories)
--   24 = Análise de tarefas (Task Analysis)
--   5  = Reforçamento diferencial (DRA)
--   17 = Dicas / Prompting (Prompt Fading)
--   27 = Videomodelação (Video Modeling)
...
VALUES
  ...
  ('Contato Visual sob Demanda', 'comunicacao',
   ...
   7, 'DTT', 'discrete_trial', 80, 3, 10, 1,
  ...
  ('Ecoico Funcional', 'comunicacao',
   ...
   7, 'DTT', 'discrete_trial', 80, 3, 10, 2,
  ...
  ('Pedido com PECS Fase III', 'comunicacao',
   ...
   2, 'PECS', 'discrete_trial', 80, 3, 10, 3,
  ...
  ('Revezamento em Jogos', 'social',
   ...
   22, 'Social Stories', 'discrete_trial', 80, 3, 10, 3,
  ...
  ('Lavar as Mãos — Cadeia Completa', 'autocuidado',
   ...
   24, 'Task Analysis', 'task_analysis', 80, 3, 10, 2,
  ...
  ('Transição entre Atividades', 'comportamento',
   ...
   17, 'Prompt Fading', 'discrete_trial', 80, 3, 10, 2,
  ...
  ('Categorização por Função', 'cognitivo',
   ...
   7, 'DTT', 'discrete_trial', 80, 3, 10, 3,
  ...
  ('Brincar Funcional com Objetos', 'brincar',
   ...
   14, 'NET', 'discrete_trial', 80, 3, 10, 2,
...
-- Corrigir seeds antigos (migration 005) que podem ter ebp_practice_id NULL
UPDATE protocol_library SET ebp_practice_id = 7
  WHERE ebp_practice_id IS NULL AND ebp_practice_name IN ('DTT', 'Discrete Trial Training', 'discrete_trial_training');
UPDATE protocol_library SET ebp_practice_id = 11
  WHERE ebp_practice_id IS NULL AND ebp_practice_name IN ('FCT', 'Functional Communication Training');
UPDATE protocol_library SET ebp_practice_id = 22
  WHERE ebp_practice_id IS NULL AND ebp_practice_name IN ('Social Stories', 'Social Skills Training');
UPDATE protocol_library SET ebp_practice_id = 5
  WHERE ebp_practice_id IS NULL AND ebp_practice_name IN ('DRA', 'Differential Reinforcement');

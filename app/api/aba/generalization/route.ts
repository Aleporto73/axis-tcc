    ...
    const result = await withTenant(async ({ client, tenantId }) => {
      return await client.query(
        `SELECT gp.*, lp.title as protocol_title FROM generalization_probes gp
         ...
         WHERE gp.protocol_id = $1::uuid AND gp.tenant_id = $2::uuid
  ...
  try {
    ...
    const result = await withTenant(async ({ client, tenantId, userId }) => {
      const proto = await client.query(
        'SELECT id, learner_id, status, mastery_criteria_pct FROM learner_protocols WHERE id = $1::uuid AND tenant_id = $2::uuid',
      ...
      const insert = await client.query(
        `INSERT INTO generalization_probes (tenant_id, protocol_id, learner_id, variation_number, context_number, variation_desc, context_desc, trials_total, trials_correct, score_pct, prompt_level, notes, evaluated_by)
         VALUES ($1::uuid,$2::uuid,$3::uuid,$4::int,$5::int,$6::text,$7::text,$8::int,$9::int,$10::numeric,$11::text,$12::text,$13::text) RETURNING *`,
      ...
      const allCells = await client.query(
        `SELECT DISTINCT ON (variation_number, context_number) variation_number, context_number, score_pct
         FROM generalization_probes WHERE protocol_id = $1::uuid AND tenant_id = $2::uuid
      ...
      if (gridComplete) {
        await client.query(
          `UPDATE learner_protocols SET status = $1, mastered_validated_at = NOW(), updated_at = NOW() WHERE id = $2::uuid AND tenant_id = $3::uuid`,
        ...
        await client.query(
          `INSERT INTO axis_audit_logs (tenant_id, user_id, actor, action, entity_type, metadata, created_at)
           VALUES ($1::uuid,$2::text,'system','GENERALIZATION_GRID_COMPLETE','learner_protocols',
           jsonb_build_object('protocol_id',$3::text,'cells_passed',6,'criteria_pct',$4::numeric,'generalization_score',100),NOW())`,

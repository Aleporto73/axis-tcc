...
// =====================================================
// AXIS ABA - Seletor de Clínica (Multi-Tenant)
//
// Exibida quando o usuário tem profiles em múltiplos tenants.
// Cards com nome da clínica, perfil do usuário, e contadores.
// Ao clicar → seta cookie → redireciona para /aba
// =====================================================
  ...
  useEffect(() => {
    async function fetchTenants() {
      try {
        ...
        // Se só tem 1 tenant, selecionar automaticamente
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          <div className="w-14 h-14 rounded-2xl bg-aba-500/10 flex items-center justify-center mx-auto mb-4">
            <span className="text-2xl font-bold text-aba-500">A</span>
          </div>
          <h1 className="text-xl font-semibold text-slate-800">Qual clínica você quer acessar?</h1>
          <p className="text-sm text-slate-400 mt-1">Você tem acesso a mais de uma clínica no AXIS ABA</p>
        </div>
            ...
            <button
              ...
              className={`w-full text-left p-5 rounded-2xl border-2 bg-white transition-all ${
                selecting === t.tenant_id
                  ? 'border-aba-500 shadow-md'
                  : 'border-slate-200 hover:border-aba-500/50 hover:shadow-sm'
              } disabled:opacity-60`}
            >
              <div className="flex items-start justify-between">
                ...
                {selecting === t.tenant_id ? (
                  <div className="w-5 h-5 border-2 border-aba-500 border-t-transparent rounded-full animate-spin" />
                ) : (
              ...
              <div className="flex gap-4 mt-3 ml-[52px] text-[11px] text-slate-400">
                <span>{t.learner_count} aprendiz{t.learner_count !== 1 ? 'es' : ''}</span>

...
const bandColors: Record<string,string> = { critico:'#ef4444', alerta:'#f59e0b', moderado:'#3b82f6', bom:'#22c55e', excelente:'#10b981' }
function getBand(c: number) { if(c<40) return 'critico'; if(c<55) return 'alerta'; if(c<70) return 'moderado'; if(c<85) return 'bom'; return 'excelente' }
function getBandLabel(c: number) { if(c<40) return 'Critico'; if(c<55) return 'Alerta'; if(c<70) return 'Moderado'; if(c<85) return 'Bom'; return 'Excelente' }

function MiniChart({ data, width=600, height=180 }: { data: CSOPoint[]; width?: number; height?: number }) {
  ...
  const pts = data.map((d,i) => ({ x: pad.l+(data.length===1?cW/2:(i/(data.length-1))*cW), y: pad.t+cH-(d.cso_aba/100)*cH, cso:d.cso_aba, date:d.session_date }))
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
      ...
      {pts.map((p,i) => <g key={i}><circle cx={p.x} cy={p.y} r={4} fill="white" stroke={bandColors[getBand(p.cso)]} strokeWidth={2}/>{data.length<=10 && <text x={p.x} y={pad.t+cH+15} textAnchor="middle" fontSize={8} fill="#94a3b8">{new Date(p.date).toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})}</text>}</g>)}
    </svg>
  )
}
...
export default function DashboardABAPage() {
  ...
  useEffect(() => {
    fetch('/api/aba/learners').then(r=>r.json()).then(d => {
      const list = d.learners||[]; setLearners(list)
  ...
  useEffect(() => {
    if(!sel) return
    Promise.all([
      fetch(`/api/aba/learners/${sel}/cso-history`).then(r=>r.json()),
      fetch(`/api/aba/sessions?learner_id=${sel}`).then(r=>r.json()),
      fetch(`/api/aba/protocols?learner_id=${sel}`).then(r=>r.json()),
    ]).then(([c,s,p]) => {
      setCso(c.history||[])
      const ss=s.sessions||[]; setSStats({total:ss.length, completed:ss.filter((x:any)=>x.status==='completed').length, scheduled:ss.filter((x:any)=>x.status==='scheduled').length})
      const pp=p.protocols||[]; setPStats({total:pp.length, active:pp.filter((x:any)=>x.status==='active').length, mastered:pp.filter((x:any)=>x.status==='mastered').length, maintained:pp.filter((x:any)=>x.status==='maintained').length})
    }).catch(()=>{})
  }, [sel])
  ...
  if(loading) return <div className="flex items-center justify-center min-h-[50vh]"><p className="text-slate-400 text-sm animate-pulse">Carregando...</p></div>
    ...
    <>
      ...
      <div className="flex justify-center pt-5 mb-6 px-4">
        <nav className="inline-flex items-center gap-1 bg-white border border-slate-200 rounded-full px-3 md:px-5 py-1.5 shadow-sm">
          <Link href="/aba/dashboard" className="px-3 py-1 text-sm font-medium text-aba-500">Dashboard</Link>
      ...
      <div className="px-4 md:px-8 lg:px-12 xl:px-16">
        <div className="flex items-center justify-between mb-5">
          ...
          {learners.length > 0 && (
            <div className="relative inline-flex items-center gap-2 bg-white border border-gray-300 rounded-xl px-4 py-2 shadow-sm">
              ...
              <select
                ...
                className="appearance-none bg-transparent text-sm font-medium text-slate-700 pr-6 cursor-pointer focus:outline-none"
              >
                {learners.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
              </select>
              <svg className="w-4 h-4 text-slate-400 absolute right-3 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </div>
          )}
        </div>
        ...
        <div className="p-4 rounded-xl border border-slate-200 bg-white">
          ...
          <div className="flex gap-3 flex-wrap">
            {[{l:'Critico',r:'0-39',c:'#ef4444'},{l:'Alerta',r:'40-54',c:'#f59e0b'},{l:'Moderado',r:'55-69',c:'#3b82f6'},{l:'Bom',r:'70-84',c:'#22c55e'},{l:'Excelente',r:'85-100',c:'#10b981'}].map(b=>(
              <div key={b.l} className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full" style={{backgroundColor:b.c}}/><span className="text-[11px] text-slate-500">{b.l} ({b.r})</span></div>
            ))}
          </div>
        </div>
        <p className="text-[10px] text-slate-300 text-center mt-8 mb-4">AXIS ABA - Motor CSO-ABA v2.6.1</p>
      </div>
    </>
  )
}

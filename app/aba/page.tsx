        ...
        {alerts.length > 0 && (
          <section className="mb-6 md:mb-8">
            ...
            <div className="space-y-2">
              {alerts.map((a, i) => {
                const isProbe = a.type === 'maintenance_probe'
                const href = a.link || '/aba/aprendizes/' + a.learner_id
                return (
                  <Link key={i} href={href} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${isProbe ? 'border-cyan-200 bg-cyan-50/50 hover:bg-cyan-50' : 'border-red-200 bg-red-50/50 hover:bg-red-50'}`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isProbe ? 'bg-cyan-100' : 'bg-red-100'}`}>
                      {isProbe ? (
                        <svg className="w-4 h-4 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs ${isProbe ? 'text-cyan-600' : 'text-red-500'}`}>{a.message}</p>
